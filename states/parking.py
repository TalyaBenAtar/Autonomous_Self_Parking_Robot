...
def final_safety_check():
    ...
    if rear_distance <= config.MIN_REAR_DISTANCE_MM:
        print("FAILED: rear too close")
        return False

    if side_distance <= config.MIN_SIDE_DISTANCE_MM:
        print("FAILED: side too close")
        return False

    print("Parking safety check passed")
    return True
...
def perform_parallel_parking(gap_length):
    """
    Parallel parking:
    1. Move slightly forward to entry position.
    2. Reverse diagonally into the gap.
    3. Counter-turn to straighten.
    4. Reverse while correcting side distance.
    ...
    drive_forward_for_motor_angle(config.PARALLEL_ENTRY_FORWARD_ANGLE)

    motion_control.turn_in_place_with_gyro(config.PARALLEL_FIRST_TURN_ANGLE)

    success = reverse_until_rear_distance_with_side_alignment(
        config.PARALLEL_FIRST_REAR_TARGET_MM,
        config.PARALLEL_SIDE_TARGET_MM,
        config.MAX_PARALLEL_FIRST_REVERSE_ANGLE
    )

    if not success:
        print("Parallel parking failed during first reverse")
        return False
